  ...
  internal static class SslPemCertificateValidator
  {
    public static void ValidateCertificate(X509Chain chain, MySqlBaseConnectionStringBuilder settings)
    {
      if (settings.SslMode >= MySqlSslMode.VerifyCA)
      {
        ...
        {
          var cert = new X509CertificateParser().ReadCertificate(x509ChainElement.Certificate.GetRawCertData());
          VerifyDates(cert);
        }
        ...
        VerifySignature(sslCA, new X509CertificateParser().ReadCertificate(serverCertificate.GetRawCertData()));
      }
